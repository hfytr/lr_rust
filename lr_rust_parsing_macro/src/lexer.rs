...
pub fn process_productions(
    ...
    let mut f = std::fs::File::create("lr_rust.parser").expect("Failed to open log file.");
    writeln!(f, "{}", LOG_PREAMBLE).expect("Fail to write file");
    dfa.write_to_file(&mut f, &id_productions).expect("Failed to write log file.");
    f.flush().expect("Failed to flush log file");
    match maybe_parser {
        Err(conflicts) => {

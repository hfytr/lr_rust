...
use syn::{
    Error, ExprClosure, Ident, LitStr, Token, Variant, parenthesized,
...
const LOG_PREAMBLE: &'static str = r#"
...
impl Production {
    pub fn get_variant(&self) -> Option<(&Variant, &str)> {
        match self {
            Production::Lexeme { name_raw, variant, .. } => Some((&variant, name_raw.as_str())),
            Production::Rule { name_raw, variant, .. } => Some((&variant, name_raw.as_str())),
            Production::None { name_raw, variant, .. } => Some((&variant, name_raw.as_str())),
...
impl Parse for Production {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        let variant: Option<Variant> = if input.parse::<Token![_]>().is_err() {
            Some(input.parse::<Variant>().context(ERR_PROD_NO_OUT_TYPE)?)
        } else {
            None
        };
        if input.peek(Token![,])
        ...
        {
            return Ok(Production::None { name_raw: variant.ident.to_string(), variant });
        }
        ...
        match prod_type_str.as_str() {
            "Literal" | "Regex" => {
                ...
                if let Some(variant) = variant {
                    Ok(Production::Lexeme {
                        name_raw: variant.ident.to_string(),
                        variant,
                        patt,
                        callback,
                        literal,
            ...
            "Rule" => {
                ...
                let variant = variant.expect(ERR_UPDATE_RULE);
                Ok(Production::Rule { name_raw: variant.ident.to_string(), variant, rules, error })
            }
...
pub fn process_productions(
...
) -> (RegexDFA, Trie, ParseTable, Vec<String>) {
    let mut trie = Trie(vec![TrieNode { fin: None, children: [None; 256] }]);
    fn update_ids<'a>(production_ids: &mut HashMap<&'a str, usize>, production: &'a str) -> usize {
        production_ids.get(production).map(|x| *x).unwrap_or_else(|| {
    ...
    let (regexi, production_ids, mut is_token, _) = productions.into_iter().fold(
        (vec![], HashMap::new(), vec![false; productions.len()], 0),
        |(mut regexi, mut production_ids, mut is_token, mut lexeme_i), production| {
            match production {
                ...
                Production::Rule { name_raw, .. } => {
                    ...
                    production_ids.insert(name_raw, production_ids.len());
                }
    ...
    let mut id_productions = std::iter::once((production_ids.len(), "EOF"))
        .chain(production_ids.iter().map(|(s, i)| (*i, *s)))
        .collect::<Vec<_>>();
    id_productions.sort_by_key(|(i, _)| *i);
    let id_productions = id_productions.into_iter().map(|(_, x)| x.to_string()).collect::<Vec<_>>();
    ...
    for (raw_components, nonterminal, error) in productions
        ...
        .flat_map(|(raw_components, rule_name, error)| {
            raw_components.iter().map(move |raw_components| (raw_components, rule_name, error))
        })
    {
    ...
    let (maybe_parser, dfa) = ParseTable::from_rules(rules, error_ids);
    let mut f = std::fs::File::create("lr_rust.parser").expect("Failed to create file \"lr_rust.parser\".");
    writeln!(f, "{}", LOG_PREAMBLE).expect("Fail to write .parser file");
    dfa.write_to_file(&mut f, &id_productions).expect("Failed to write .parser file.");
    f.flush().expect("Failed to flush .parser file");
    match maybe_parser {
        Err(conflicts) => {
            conflicts.iter().for_each(|c| c.pprint(&id_productions));
            exit(1);
        }
        Ok(parser) => (regex, trie, parser, id_productions),
    }
}

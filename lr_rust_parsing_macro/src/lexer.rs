...
At the bottom of the file are all the actions taken by the parser, and the parsing state at each step (Search for "PARSER ACTIONS" to get there).
...
pub fn process_productions(
    ...
    let id_productions = id_productions
        .into_iter()
        .map(|(_, x)| x.to_string())
        .collect::<Vec<_>>();
    ...
    let mut f = std::fs::File::create("lr_rust.log").expect("Failed to open log file.");
